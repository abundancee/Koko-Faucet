...
export function TransactionItem({ item, explorerBaseUrl, tokenSymbol }: TransactionItemProps) {
  return (
    <motion.article
      ...
      className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl border border-border-muted bg-bg-card p-4 shadow-sm transition-colors duration-300 hover:border-brand-accent/40 hover:shadow-md"
    >
      <div className="space-y-1 flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <p className="text-sm font-semibold text-primary break-all">{shortenAddress(item.wallet)}</p>
      ...
      <div className="text-right flex-shrink-0 mt-2 sm:mt-0">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="mt-1 inline-flex items-center gap-1 text-xs text-secondary transition-colors duration-300 hover:text-brand-accent dark:hover:text-brand-secondary break-all"
